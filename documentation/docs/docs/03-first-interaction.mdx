...
:::caution
If you're getting a message along the lines of "The server is not hosting the websocket. Try enabling it in the config", please refer to [Configuring the Web Panel](Installation-guide#configuring-the-web-panel)
:::
...
To create static entries, we must make a special page called a `Static` page. Let's create a static page called `Flower Static`. Then, inside of it, let's create a `simple_speaker` which is the most
...
<ReactPlayer url={require("./assets/first-interaction/select-speaker.webm").default} playing loop width="100%" height="100%" />
...
Another way of connecting entries is shown in the video below:

<ReactPlayer url={require("./assets/first-interaction/connect-entries-alternative.webm").default} playing loop width="100%" height="100%" />

This method is oftentimes quicker, as it connects entries as soon as they are created.
...
To create a fact, head over to a static page such as `Flower Static`, click on the `+` button in the app bar, and add a `permanent_fact` entry. Then change the name
...
the sequential times. Rename it to `smell_flower_second_time`, add the text `You smell the flower again. It still smells <blue><bold>fragrant</bold></blue>.`, select our `flower` speaker we created earlier, and finally specify a
...
Now that you understand the basics of TypeWriter and have made your first interaction, it's time to spice up your interaction by adding a `cinematic`. Read the [First Interaction](First-cinematic) guide to learn more! If you have any questions, feel free to ask them in the [Discord](https://discord.gg/HtbKyuDDBw).
