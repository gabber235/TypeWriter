...
a command. Later in the guide we'll continue the [First Interaction](03-first-interaction) guide to trigger the cinematic
from a flower.
...
Let's do that by modifying the flower example from [First Interaction](03-first-interaction) so that one of the
