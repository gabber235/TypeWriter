...
:::info
This guide builds upon the [Interactions](./01-interactions.mdx) guide.
...
a command. Later in the guide we'll continue the [Interactions](./01-interactions.mdx) guide to trigger the cinematic
from a flower.
...
Let's do that by modifying the flower example from [Interactions](./01-interactions.mdx) so that one of the
