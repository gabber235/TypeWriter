...
:::info
This guide builds upon the [Interactions](./interactions) guide.
...
a command. Later in the guide we'll continue the [Interactions](./01-interactions) guide to trigger the cinematic
from a flower.
...
Let's do that by modifying the flower example from [Interactions](./interactions) so that one of the
