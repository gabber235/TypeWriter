...
|------------|-------------|------------|
| Message | Display a single message to the player. | `text` - The message to display. <br /> `speaker` - The speaker of the message. |
| Random Message | Display a random message from a list to the player. | `messages` - A list of messages to display. <br /> `speaker` - The speaker of the message. |
| Spoken | Display an animated message to the player. | `text` - The message to display. <br /> `speaker` - The speaker of the message. <br /> `duration` - The time for each character to be written. |
| Random Spoken | Display a random animated message from a list to the player. | `messages` - A list of messages to display. <br /> `speaker` - The speaker of the message. <br /> `duration` - The time for each character to be written. |
| Option | Display a list of options to the player. | `text` - The message to display. <br /> `options` - A list of options to display. <br /> `speaker` - The speaker of the message. |
...
| On Run Command | Triggered when a user runs a command. This command will be registered to the server. | `command` - The command to be ran. Must not include the slash. |
| On Detect Command Ran | Triggered when an already existing command is run. The command must already be registered to the server. | `command` - The command to be ran. Must not include the slash. |
...
| On Interact With Block | Triggered when a player interacts (R-Click) with a block. | `block` - The block that must be interacted with. <br /> `location` (optional) - Where the block should be. <br /> `itemInHand` (optional) - The item that must be in the player's hand. |
...
| Console Run Command | Run a command from the console. | `command` - The command to be ran. Must not include the slash. |
| Player Run Command | Run a command from the player. | `command` - The command to be ran. Must not include the slash. |
| Play Sound | Plays a sound. | `sound` - The sound to play. <br /> `volume` - The volume of the sound. <br /> `pitch` - The pitch of the sound <br /> `location` (optional) - Where to play the sound from. |
...
| Give Item | Gives the player an item. | `material` - The item to give. <br /> `amount` - The size of the stack. <br /> `displayName` (optional) - Sets the display name for the item. If not set, the item will have it's normal name. <br /> `lore` - The lore of the item. If not set, the item will have it's normal lore if any. |
| Drop Item | Drops an item to the player. | `material` - The item to drop. <br /> `amount` - The size of the stack. <br /> `displayName` (optional) - Sets the display name for the item. If not set, the item will have it's normal name. <br /> `lore` - The lore of the item. If not set, the item will have it's normal lore if any. <br /> `location` (optional) - The location to drop the item. |
| Delayed Action | Delay the next trigger. | `duration` - The time to wait. |

### Static

Static entries are for holding data. They are used as parameters for other entries. All static entries contain a field called `comment`. It does nothing, and serves as a tool for you.
