import { blueprints, CustomField, Entry, FieldInfo, findEntry, getModifier, ListField, MapField, ObjectField, Page, PrimitiveField } from "@site/src/components/EntryDisplay/data";
...
export default function EntryNodeInspector({ entryId, pages }: { entryId: string, pages: Page[] }) {
...
export function EntryInspector({ entry, pages }: { entry: Entry, pages: Page[] }) {
...
function FieldHeader({ name }: { name: string }) {
    ...
    return <div className="text-gray-700 dark:text-gray-300 text-sm">{format(display)}</div>;
}

function SimpleValueField({ name, value, icon }: { name?: string, value: any, icon: string }) {
...
function FieldInspector({ fieldInfo, path, value, pages }: { fieldInfo: FieldInfo, path: string, value: any, pages: Page[] }) {
    ...
    if (type === "object") {
        return <div className="flex flex-col space-y-1 w-full">
            ...
            <ObjectFieldInspector fieldInfo={fieldInfo as ObjectField} path={path} value={value} pages={pages} />
        </div>;
    }
    if (type === "custom") {
...
function ListFieldInspector({ fieldInfo, path, value, pages }: { fieldInfo: ListField, path: string, value: any[], pages: Page[] }) {
    ...
    return <div className="transition-all duration-300 ease-in-out">
        <div className="flex space-x-1 items-center p-1 hover:bg-black hover:bg-opacity-10 dark:hover:bg-opacity-40 rounded-md cursor-pointer" onClick={() => setIsOpen(!isOpen)}>
            <Icon icon={"mdi:chevron-right"} className="w-5 h-5 transition-transform duration-200 ease-in-out transform-gpu" style={{ transform: `rotate(${isOpen ? 90 : 0}deg)` }} />
            <FieldHeader name={path} />
            {!isOpen && <div className="w-full text-gray-500 dark:text-gray-400 text-xs">
                ({value.length} item{value.length != 1 ? "s" : ""})
            </div>}
        </div>
        {isOpen && <div className={`overflow-hidden pl-3 space-y-3`}>
...
function ObjectFieldInspector({ fieldInfo, path, value, pages, ignoreFields = [] }: { fieldInfo: ObjectField, path: string, value: any, pages: Page[], ignoreFields?: string[] }) {
    ...
    return <div className={`space-y-3 ${path.length > 0 ? "pl-2" : ""}`}>
...
function EntryReferenceField({ entryId, pages, path }: { entryId: any, pages: Page[], path: string }) {
    if (!entryId) {
        return <div className="flex flex-col space-y-1">
            <FieldHeader name={path} />
            <SimpleValueField value="No Entry Referenced" icon="material-symbols:link-off" />
        </div>;
    }
    const entry = findEntry(pages, entryId);
    ...
    return <div className="flex flex-col space-y-1">
        <FieldHeader name={path} />
        <div className="text-white text-xs rounded-md p-2" style={{ backgroundColor: blueprint.color }}>
...
function SoundSourceField({ type, entryId, location, pages, path }: SoundSourceFieldProps) {
    if (type === "self") {
...
function OptionalField({ value, field, path, pages }: OptionalFieldProps) {
    ...
    const seperator = path.length > 0 ? "." : "";
    return <div className={"flex flex-row w-full" + (enabled ? "" : " opacity-50 cursor-not-allowed")}>
