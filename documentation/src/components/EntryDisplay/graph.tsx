...
import { blueprintFieldsWithModifier, blueprints, Entry, getAll, Page } from "./data";
import { format } from '.';
import { Icon } from "@iconify/react";

function getEdges(entry: Entry, acceptedTag: string, entries: Entry[]): NodeEdge[] {
...
function getLayoutedElements(nodes: Node<EntryNodeProps, string>[], edges: Edge<any>[], options: GraphLabel): { nodes: Node<EntryNodeProps, string>[], edges: Edge<any>[] } {
...
export function PageLayout({ page }: PageLayoutProps) {
    ...
    const [nodes, setNodes, onNodesChange] = useNodesState(page.entries.map((entry) => ({ id: entry.id, type: "custom", position: { x: 0, y: 0 }, data: { entry, direction } as EntryNodeProps })));
    const [edges, setEdges, onEdgesChange] = useEdgesState(page.entries.flatMap((entry) => getEdges(entry, tag, page.entries)));

    const hasSizes = nodes.some((node) => node.width !== undefined && node.height !== undefined);

    useEffect(() => {
        const { nodes: layoutedNodes, edges: layoutedEdges } = getLayoutedElements(nodes, edges, { rankdir: direction });
    ...
    return <div className="h-[35em] w-full">
        <ReactFlow
            nodes={nodes}
            edges={edges}
...
function EntryNode({ data, selected }: NodeProps<EntryNodeProps>) {
    ...
    return <>
        ...
        <Handle type="source" position={direction === "TB" ? Position.Bottom : Position.Right} className="opacity-0" />
    </>;
}
