...
class ElementValueMutator(
    ...
    private fun applyOne(
        ...
        runCatching {
            ...
            when (mutation) {
                is ElementValueMutation.SetValue -> {
                    replace(graph, stored, target, mutation.value)
                }
                ...
                is ElementValueMutation.InsertListItems -> {
                    insertList(graph, stored, target, mutation.index, mutation.values)
                }

                is ElementValueMutation.RemoveListItems -> {
                    removeList(stored, target, mutation.index, mutation.count)
                }
                ...
                is ElementValueMutation.PutMapEntries -> {
                    putMap(graph, stored, target, mutation.entries)
                }

                is ElementValueMutation.RemoveMapEntries -> {
                    removeMap(stored, target, mutation.keys)
                }
            }
        }.getOrNull()
...
private fun TypeExpression.requireListType(
    graph: TypeGraph,
    logical: DataValue,
): TypeExpression.ListType = materialize(graph, logical) as TypeExpression.ListType

private fun TypeExpression.requireMapType(
    graph: TypeGraph,
    logical: DataValue,
): TypeExpression.MapType = materialize(graph, logical) as TypeExpression.MapType

private fun TypeExpression.materialize(
    graph: TypeGraph,
    logical: DataValue,
): TypeExpression {
    var current = this
    ...
    buildSet {
        fun visit(value: DataValue) {
            ...
            when (value) {
                ...
                is DataValue.Record -> {
                    value.fields.values.forEach(::visit)
                }
