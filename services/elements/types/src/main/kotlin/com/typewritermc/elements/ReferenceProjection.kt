...
internal class ProjectionContext(
    ...
    fun transform(
        ...
        when (expression) {
            ...
            is TypeExpression.Named -> transformNamed(expression.reference, value, reference, parameters)
    ...
    fun materialize(
        ...
        when (expression) {
            is TypeExpression.Parameter -> parameters[expression.name]?.let { materialize(it, parameters) } ?: expression
            is TypeExpression.ListType -> expression.copy(element = materialize(expression.element, parameters))
            is TypeExpression.MapType ->
                expression.copy(
            ...
            is TypeExpression.Named ->
                expression.copy(reference = expression.reference.withArguments(expression.reference.arguments.map { materialize(it, parameters) }))
...
private fun DataValue.requireStringReference(): ResourceId =
    ResourceId.parse((this as DataValue.StringValue).value)
