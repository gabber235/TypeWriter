...
object SegmentModifierComputer : StaticModifierComputer<Segments> {
	...
	override fun computeModifier(annotation: Segments, info: FieldInfo): FieldModifier? {
		if (info !is ListField) {
			plugin.logger.warning("Segment annotation can only be used on lists")
			return null
		}
		val type = info.type
		if (type !is ObjectField) {
			plugin.logger.warning("Segment annotation can only be used on lists of objects")
			return null
		}
		...
		if (startFrame !is PrimitiveField || endFrame !is PrimitiveField) {
			plugin.logger.warning("Segment annotation can only be used on lists of objects with startFrame and endFrame fields")
			return null
		}

		if (startFrame.type != PrimitiveFieldType.INTEGER || endFrame.type != PrimitiveFieldType.INTEGER) {
			plugin.logger.warning("Segment annotation can only be used on lists of objects with startFrame and endFrame fields of type int")
			return null
		}
