...
class PositionComponent(
    ...
    private fun createUpdateEntityPositionAndRotationPacket(location: Location): PacketContainer =
        createUpdateEntityPositionAndRotationPacket(
            entityId,
            this.location,
            location
        )
    ...
    private fun createRotateHeadPacket(yaw: Float): PacketContainer {
...
fun createUpdateEntityPositionAndRotationPacket(
    entityId: Int,
    ...
    packet.integers.write(0, entityId)
    packet.shorts.write(0, oldLocation.x.calculateDelta(newLocation.x))
    packet.shorts.write(1, oldLocation.y.calculateDelta(newLocation.y))
    packet.shorts.write(2, oldLocation.z.calculateDelta(newLocation.z))

    packet.bytes.write(0, newLocation.yaw.convertRotation())
    packet.bytes.write(1, newLocation.pitch.convertRotation())

    packet.booleans.write(0, false)
    return packet
}

fun Double.calculateDelta(new: Double): Short {
