// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'heading.dart';

// **************************************************************************
// RiverpodGenerator
// **************************************************************************

String _$entryIdHash() => r'aae409d674bee6b65c432d6810f2f3ad299e8a26';

/// See also [_entryId].
@ProviderFor(_entryId)
final _entryIdProvider = AutoDisposeProvider<String>.internal(
  _entryId,
  name: r'_entryIdProvider',
  debugGetCreateSourceHash:
      const bool.fromEnvironment('dart.vm.product') ? null : _$entryIdHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef _EntryIdRef = AutoDisposeProviderRef<String>;
String _$entryNameHash() => r'5b97f6d0bfdb17369f3c4fec5aafcba45e1244a0';

/// See also [_entryName].
@ProviderFor(_entryName)
final _entryNameProvider = AutoDisposeProvider<String>.internal(
  _entryName,
  name: r'_entryNameProvider',
  debugGetCreateSourceHash:
      const bool.fromEnvironment('dart.vm.product') ? null : _$entryNameHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef _EntryNameRef = AutoDisposeProviderRef<String>;
String _$entryTypeHash() => r'b8db383dacb169526f823865fb2c90e478a4b4ee';

/// See also [_entryType].
@ProviderFor(_entryType)
final _entryTypeProvider = AutoDisposeProvider<String>.internal(
  _entryType,
  name: r'_entryTypeProvider',
  debugGetCreateSourceHash:
      const bool.fromEnvironment('dart.vm.product') ? null : _$entryTypeHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef _EntryTypeRef = AutoDisposeProviderRef<String>;
String _$entryUrlHash() => r'694a800667fd4e4be38e5ae6535f605833cda12e';

/// See also [_entryUrl].
@ProviderFor(_entryUrl)
final _entryUrlProvider = AutoDisposeProvider<String>.internal(
  _entryUrl,
  name: r'_entryUrlProvider',
  debugGetCreateSourceHash:
      const bool.fromEnvironment('dart.vm.product') ? null : _$entryUrlHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef _EntryUrlRef = AutoDisposeProviderRef<String>;
String _$entryColorHash() => r'7bdd0ca948f969fd17f3b33c020f20585847b543';

/// See also [_entryColor].
@ProviderFor(_entryColor)
final _entryColorProvider = AutoDisposeProvider<Color>.internal(
  _entryColor,
  name: r'_entryColorProvider',
  debugGetCreateSourceHash:
      const bool.fromEnvironment('dart.vm.product') ? null : _$entryColorHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef _EntryColorRef = AutoDisposeProviderRef<Color>;
// ignore_for_file: type=lint
// ignore_for_file: subtype_of_sealed_class, invalid_use_of_internal_member, invalid_use_of_visible_for_testing_member
